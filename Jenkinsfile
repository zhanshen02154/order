					...
					try {
						...
						withCredentials([string(credentialsId: 'kubernetes-api-server', variable: 'k8s_api_server')]) {
							withKubeConfig([credentialsId: 'kubernetes-config', serverUrl: "$k8s_api_server", namespace: 'dev']) {
								sh '''
								set +x
								lastReversion=$(/usr/bin/kubectl rollout history deployment/order-service -n dev | awk '{print $1}' | tail -n 2)
								/usr/bin/kubectl rollout undo deployment/order-service -n dev --to-revision=$lastReversion
								'''
							}
						}
					}
